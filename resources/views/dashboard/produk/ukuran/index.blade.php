                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="row card-header py-3 justify-content-between">
                                <div class="col-6">
                                    <h5 class="m-0 font-weight-bold text-primary">Ukuran Produk</h5>
                                </div>
                                <div class="col-6 text-end">
                                    <a href="{{ url('dashboard/produk/'.$produk_id . '/ukurans/create') }}" class="btn-sm  btn btn-primary">
                                        Tambah ukuran</a>
                                </div>
                            </div>
                                ...
                                <tbody>
                                   ...
                                   <tr>
                                    <td class="text-center">{{ $ukurans->firstItem() + $i }}</td>
                                      ...
                                      <td class="text-center">{{ $h->ukuran->jenis_ukuran }}</td>
                                      <td class="text-center">{{ $h->stock }}</td>

                                      <td>   <div class="d-flex">
                                               <a href="{{ url('/dashboard/produk/'.$produk->id.'/ukurans/'.$h->id.'/edit') }}" class="btn btn-info me-2">
                                                   Edit
                                               </a>
                                            {{-- <form method="POST" action="{{ url('/dashboard/produk/'.$produk->id.'/ukurans/'.$h->id ) }}">
                                                   @method('DELETE')
                                                   @csrf
    ...
    <script>
        $('table').on('click', '.hapus', function() {
                    ...
                    Swal.fire({
                        ...
                        preConfirm: () => {
                            return $.ajax({
                                url:'{{ url("/dashboard/produk/".$produk->id. "/ukurans") }}/' +id,
                                method: 'POST',
                                headers: {
